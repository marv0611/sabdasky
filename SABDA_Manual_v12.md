...
This is v12. It incorporates all v8/v9/v10 content plus: **unified preview/render HTML** (single file with built-in 360° room viewer, press R; replaces separate interactive viewer), **speed controls** (Y=5×, T=30×), **loop check renderer** (render_loopcheck.js for verifying loop continuity), **bird homing fix** (gentle radial nudge replaces aggressive heading-steering that caused circular orbits), **preview mode limitations** (dt/time mismatch documentation), and **commit discipline rule** (always commit working files immediately).
...
| **v11** | **Unified preview/render HTML (built-in 360° room viewer, speed controls), loop check renderer, bird homing fix, commit discipline rule** |
...
53. **(v12.5, replaces v11 lesson) Single unified HTML for both preview and render.** The `sabda_*_slim.html` file serves as BOTH the live preview (with 360° room viewer, speed controls, orbit) AND the Puppeteer render source (via `SABDA_RENDER_FRAME`). The earlier v11 rule about separate files is obsolete — the unified file has no performance penalty because: (a) the room viewer reuses the same cubemap (zero extra render passes for content), (b) `antialias: false` (no MSAA overhead), (c) no bloom, (d) OrbitControls loaded dynamically only when R is pressed. The Puppeteer renderer calls `SABDA_RENDER_FRAME` directly and never triggers the room viewer code. **One file, two modes — press R for room, Puppeteer ignores room entirely.**

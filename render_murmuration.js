...
const HTML_FILE = path.resolve(__dirname, 'sabda_murmuration_full.html');
...
if (!fs.existsSync(HTML_FILE)) {
  console.error(`ERROR: ${HTML_FILE} not found. Run: python3 assemble_murmuration.py`);
  process.exit(1);
}
...
async function main() {
  ...
  await runFFmpeg([
    ...
    '-g', String(FPS),
    '-pix_fmt', 'yuv420p', '-movflags', '+faststart',
    path.join(OUTPUT_DIR, 'sabda_bottom.mp4')
  ]);
